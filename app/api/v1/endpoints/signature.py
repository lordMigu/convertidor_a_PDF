...
# Asegurar que el directorio de subidas existe (reutilizamos la configuración)
...
async def sign_document(
    ...
    # Leer el P12 en memoria
    p12_bytes = await p12_file.read()

    # 4. Ejecutar firma (CPU bound) en threadpool
    try:
    ...
    try:
        with open(file_path, 'rb') as f:
            ...
            if sig_status.signer_cert:
                ...
                cn = subject.get_attributes_for_oid(dict(commonName="2.5.4.3"))
                # Ajuste: obtener el valor correctamente dependiendo de la librería de crypto
                result_data["signer_name"] = str(subject.rfc4514_string()) # Fallback simple
                
                # Intentar obtener CN más limpio si es posible
